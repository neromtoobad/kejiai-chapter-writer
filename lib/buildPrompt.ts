  ...
  if (target === "all") {
    ...
    if (target === "1") {
      ...
      lines.push(
        ...
        ``,
        `**STRICT rules for 4.2 Demographic Profile — read every word:**`,
        ``,
        `1. The variables listed in the "Demographic Frequencies" sub-block of COMPUTED STATISTICS are the **only** demographic variables in this study. Build one numbered frequency table for each (caption "Table 4.X: Distribution of Respondents by <Variable>" above, "*Source: Field Survey, ${new Date().getFullYear()}*" below) and copy the counts and percentages verbatim.`,
        ``,
        `2. You may **NOT** create section headings, sub-sections, or frequency tables for demographic variables that are **not** in the Demographic Frequencies block. Examples of variables that you must NOT invent if they are absent: gender, age band, faculty, department, level of study, marital status, religion, location, platform usage, social interaction level, or any other category that "sounds like" it should be there. If it is not in the block, the data was not collected for it — say nothing about it.`,
        ``,
        `3. You may **NOT** write \`[Insert: <X> frequency distribution required]\` or any \`[Insert: …]\` placeholder for an entire demographic table. The \`[Insert: …]\` marker is only for missing values inside an existing Likert table cell (e.g. response counts per scale point that were not pre-computed). A whole missing demographic section means the data was not collected — do not flag it for insertion, just omit it.`,
        ``,
        `4. If a column with "age" semantics is **numeric**, report it as one sentence in the section narrative: *"The mean age of respondents was X.XX years (SD = Y.YY, range = min–max)."* Do not band it into age groups, do not create a frequency table for it.`,
        ``,
        `5. Skip identifier columns entirely (e.g. \`id\`, \`s_n\`, \`respondent_id\`, \`serial_no\`).`,
        ``,
        `6. If the Demographic Frequencies block is **absent or empty**, write a single paragraph for 4.2 noting that the study focused on the substantive items rather than respondent demographics, then move on to 4.3. Do not invent a demographic profile.`,
        ``,
        ``,
